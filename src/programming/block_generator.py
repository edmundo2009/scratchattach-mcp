...
class BlockGenerator:
    ...
    def __init__(self, knowledge_path: str = None, patterns_path: str = None):
        ...
        if knowledge_path is None:
            knowledge_path = self._get_default_path(
                "knowledge/scratch_blocks.json")
    ...
    def _load_knowledge_base(self, path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            print(
                f"Warning: Knowledge base not found at {path}. Using minimal defaults.")
    ...
    def _load_patterns(self, path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            print(
                f"Warning: Patterns file not found at {path}. Using built-in patterns.")
                ...
                for input_name in block_data["inputs"]:
                    ...
                    if input_name == "STEPS" or input_name == "DX" or input_name == "DY":
                        value = intent.parameters.get(
                            "steps", default_values.get(input_name, 10))
                        ...
                        block.inputs[input_name] = default_values.get(
                            input_name, "")
    ...
    def _generate_from_pattern(self, intent: Intent) -> Tuple[List[ScratchBlock], str]:
        ...
        explanation = pattern.get(
            "explanation", f"This creates a {intent.action} effect!")
        return blocks, explanation
        ...
        for block in blocks:
            ...
            for category, category_blocks in self.block_templates.items():
                if block.opcode in category_blocks:
                    kid_explanation = category_blocks[block.opcode].get(
                        "kid_explanation", block.description)
